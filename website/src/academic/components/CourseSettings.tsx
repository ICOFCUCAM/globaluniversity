...
import type { CompletionRule } from '@/academic/lib/credential/certificate';
import { Card } from '@/academic/components/ui';
...
export function CourseSettings({
  ...
  async function save(body: Record<string, unknown>) {
    ...
    try {
      const response = await fetch(`/api/studio/courses/${courseId}/settings`, {
        method: 'POST',
  ...
  return (
    <div className="grid gap-4 lg:grid-cols-2">
      {error && (
        <p className="rounded border border-red-200 bg-red-50 px-3 py-2 text-sm text-studio-bad lg:col-span-2">{error}</p>
      )}

      <Card className="px-5 py-4">
        <h2 className="flex items-center gap-2 text-sm font-semibold">
          <Tags size={16} className="text-studio-brand" /> Terms that are never changed
        </h2>
        ...
        <textarea
          ...
          rows={6}
          className="mt-3 w-full rounded-md border border-studio-page-line px-3 py-2 font-mono text-sm"
          placeholder={'Yahuah\nYahusha HaMashiach'}
        />
        <button
          type="button" disabled={busy}
          onClick={() => save({ action: 'terminology', terms: terms.split('\n') })}
          className="mt-2 rounded-md bg-studio-brand px-3.5 py-2 text-sm font-medium text-white hover:bg-studio-brand-dark disabled:opacity-50"
        >
          Save the list
        </button>
      </Card>
      ...
      <Card className="px-5 py-4">
        <h2 className="flex items-center gap-2 text-sm font-semibold">
          <Mic2 size={16} className="text-studio-brand" /> The voice this course is spoken in
        </h2>
        <p className="mt-1 text-sm text-studio-ink-soft">
        ...
        <label className="mt-3 block text-[11px] uppercase tracking-wide text-studio-ink-faint">Default</label>
        <select
          className="mt-1 w-full rounded-md border border-studio-page-line px-3 py-2 text-sm"
        ...
        <p className="mt-3 text-[11px] uppercase tracking-wide text-studio-ink-faint">Allowed on this course</p>
            ...
            return (
              <button
                ...
                className={`rounded border px-2.5 py-1 text-xs ${
                  on ? 'border-studio-brand bg-studio-brand text-white' : 'border-studio-page-line text-studio-ink-soft'
                }`}
              >
                {voice.label}
              </button>
        ...
        <p className="mt-2 text-xs text-studio-ink-faint">
      ...
      <Card className="px-5 py-4">
        <h2 className="flex items-center gap-2 text-sm font-semibold">
          <Award size={16} className="text-studio-brand" /> What completing this course means
        </h2>
        <p className="mt-1 text-sm text-studio-ink-soft">
        ...
        <div className="mt-3 grid gap-3 sm:grid-cols-2">
          {([
            ...
            <div key={key}>
              <label className="block text-[11px] uppercase tracking-wide text-studio-ink-faint">{label}</label>
              <input
              ...
              />
              <p className="mt-1 text-xs text-studio-ink-faint">{blurb}</p>
            </div>
          ))}
        </div>
        ...
        <div className="mt-3 flex flex-wrap gap-2">
          <button
            ...
            className="rounded-md bg-studio-brand px-3.5 py-2 text-sm font-medium text-white hover:bg-studio-brand-dark disabled:opacity-50"
          >
          ...
          {completion && (
            <button
              ...
              className="rounded-md border border-studio-page-line px-3.5 py-2 text-sm text-studio-ink-soft"
            >
      ...
      <Card className="px-5 py-4">
        <h2 className="flex items-center gap-2 text-sm font-semibold">
          <Coins size={16} className="text-studio-brand" /> What this course has cost to process
        </h2>
        <p className="mt-1 text-sm text-studio-ink-soft">
        ...
        {costs.runs === 0 ? (
          ...
          <>
            <dl className="mt-3 grid grid-cols-3 gap-3 text-sm">
              <div>
                <dt className="text-[11px] uppercase tracking-wide text-studio-ink-faint">Runs</dt>
                <dd className="text-lg font-semibold">{costs.runs}</dd>
              </div>
              <div>
                <dt className="text-[11px] uppercase tracking-wide text-studio-ink-faint">Across</dt>
                <dd className="text-lg font-semibold">{lectures} lectures</dd>
              </div>
              <div>
                <dt className="text-[11px] uppercase tracking-wide text-studio-ink-faint">Output</dt>
            ...
            <ul className="mt-3 space-y-1 text-xs text-studio-ink-soft">
              {costs.byStage.map((row) => (
                <li key={row.stage} className="flex justify-between border-b border-studio-page-line py-1">
            ...
            {costs.unmetered && (
              <p className="mt-3 rounded border border-studio-page-line bg-studio-page px-3 py-2 text-xs text-studio-ink-soft">
