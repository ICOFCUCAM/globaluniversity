...
import { parseFlashcards, type Flashcard } from '@/academic/lib/study/flashcards';
...
export function FlashcardDeck({
  ...
  const load = useCallback(async () => {
    try {
      const response = await fetch(`/api/studio/study?aid=${encodeURIComponent(studyAidId)}`);
  ...
  if (!all.length) {
    return <p className="text-sm text-studio-ink-soft">No cards could be read from this set.</p>;
  }
  if (!deck && !failed) {
    return (
      <p className="flex items-center gap-2 text-sm text-studio-ink-soft">
  ...
  const answer = async (knew: boolean) => {
    ...
    if (!failed) {
      try {
        const response = await fetch('/api/studio/study', {
          method: 'POST',
  ...
  if (done) {
    const when = deck?.nextDueAt
      ...
      : null;
    return (
      <div dir={dir} className="rounded-lg border border-studio-page-line bg-studio-page-card p-8 text-center">
        <p className="font-medium">
          {t('study.nothingDue')}
        </p>
        {when && (
          <p className="mt-2 flex items-center justify-center gap-1.5 text-sm text-studio-ink-soft">
        ...
        {!everything && (
          <button
            ...
            className="mt-5 rounded border border-studio-page-line px-3.5 py-2 text-xs text-studio-ink-soft hover:border-studio-brand/40"
          >
  ...
  return (
    <div dir={dir}>
      {!failed && deck && at === 0 && !everything && (
        <p className="mb-3 text-xs text-studio-ink-faint">
      ...
      <button
        ...
        className="flex min-h-[10rem] w-full flex-col items-center justify-center rounded-lg border border-studio-page-line bg-studio-page-card p-8 text-center"
      >
        <p className="text-lg font-medium">{showBack ? card.back : card.front}</p>
        <p className="mt-3 flex items-center gap-1 text-[11px] uppercase tracking-wide text-studio-ink-faint">
      ...
      <div className="mt-3 flex flex-wrap items-center justify-between gap-2">
        ...
        <div className="flex gap-2">
          <button
            ...
            className="rounded border border-emerald-300 bg-emerald-50 px-3 py-1.5 text-xs text-studio-ok"
          >
            {t('study.knewIt')}
          </button>
          <button
            ...
            className="rounded border border-studio-page-line px-3 py-1.5 text-xs text-studio-ink-soft"
          >
            {t('study.againLater')}
          </button>
        </div>
