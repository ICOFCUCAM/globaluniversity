...
import { RecordLecture } from '@/academic/components/RecordLecture';
...
export function NewLectureForm({ courseId, nextSequence }: { courseId: string; nextSequence: number }) {
  ...
  async function submit(event: React.FormEvent) {
    ...
    try {
      const response = await fetch('/api/studio/lectures', {
        method: 'POST',
      ...
      if (file) {
        ...
        const upload = await fetch('/api/studio/upload', { method: 'POST', body: carrier });
        if (!upload.ok) {
      ...
      router.push(`/academic-studio/lectures/${payload.lecture.id}`);
    } finally { setBusy(false); }
  }

  return (
    <form onSubmit={submit} className="max-w-2xl space-y-5 rounded-lg border border-studio-page-line bg-studio-page-card p-6">
      {error && <p className="rounded border border-red-200 bg-red-50 px-3 py-2 text-sm text-studio-bad">{error}</p>}
      ...
      <div>
        <label className="block text-sm font-medium">Your own framing <span className="font-normal text-studio-ink-faint">(optional)</span></label>
        <textarea
      ...
      <div className="flex items-end gap-4">
        <div>
          ...
          <input
            ...
            className="mt-1 w-28 rounded-md border border-studio-page-line px-3 py-2 text-sm"
          />
        </div>
        <RecordLecture onRecorded={(recorded) => setFile(recorded)} />

        <label className="flex cursor-pointer items-center gap-2 rounded-md border border-dashed border-studio-page-line px-3 py-2 text-sm text-studio-ink-soft hover:border-studio-brand/40">
          <Upload size={15} />
      ...
      <div>
        ...
        <textarea
          ...
          rows={10}
          className="mt-1 w-full rounded-md border border-studio-page-line px-3 py-2 font-mono text-[13px]"
        />
      </div>

      <button
        type="submit" disabled={busy}
        className="inline-flex items-center gap-2 rounded-md bg-studio-brand px-4 py-2.5 text-sm font-medium text-white disabled:opacity-50"
      >
