...
export function WordCheck({
  ...
  useEffect(() => {
    fetch(`/api/studio/artefacts/${artefactId}/words`)
      .then((r) => r.json())
      .then((payload) => {
  ...
  async function save() {
    ...
    try {
      ...
      const response = await fetch(`/api/studio/artefacts/${artefactId}/words`, {
        method: 'POST',
  ...
  if (error) {
    return <div className="rounded-md border border-red-200 bg-red-50 px-4 py-3 text-sm text-studio-bad">{error}</div>;
  }
  ...
  return (
    <div className="rounded-lg border border-studio-page-line bg-studio-page-card p-5">
      <div className="flex items-start gap-2">
        <SpellCheck size={18} className="mt-0.5 text-studio-brand" />
        <div>
      ...
      {words.length === 0 ? (
        <p className="mt-4 rounded border border-emerald-200 bg-emerald-50 px-3 py-2 text-sm text-studio-ok">
        ...
        <ul className="mt-4 space-y-3">
          {words.map((word) => (
            <li key={word.word} className="rounded border border-studio-page-line px-4 py-3">
              <div className="flex flex-wrap items-baseline justify-between gap-2">
                <p className="font-medium">
                  <mark className="bg-amber-100 px-1">{word.word}</mark>
                  <span className="ms-2 text-xs font-normal text-studio-ink-faint">
                    {word.occurrences === 1 ? 'once' : `${word.occurrences} times`}
                  </span>
                </p>
                <p className="flex items-center gap-1 text-[11px] uppercase tracking-wide text-studio-warn">
                  <AlertTriangle size={12} /> {REASON[word.reason]}
                </p>
              </div>

              {word.contexts.slice(0, 2).map((context, i) => (
                <p key={i} className="mt-1 text-xs italic text-studio-ink-soft">“{context}”</p>
              ))}
              ...
              <div className="mt-2 flex flex-wrap items-center gap-2">
                {word.suggestion && (
                  <button
                    ...
                    className="rounded border border-studio-brand/40 bg-studio-brand-tint px-2 py-1 text-xs text-studio-brand-dark"
                  >
                ...
                <input
                  ...
                  className="w-52 rounded border border-studio-page-line px-2 py-1 text-xs"
                />
                <button
                  type="button"
                  ...
                  className={`rounded border px-2 py-1 text-xs ${
                    (choices[word.word] ?? '') === ''
                      ...
                      : 'border-studio-page-line text-studio-ink-soft'
                  }`}
                >
      ...
      <button
        ...
        className="mt-4 inline-flex items-center gap-2 rounded-md bg-studio-brand px-4 py-2 text-sm font-medium text-white disabled:opacity-50"
      >
