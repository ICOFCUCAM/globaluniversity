...
import { direction, LANGUAGES, LANGUAGE_BY_CODE } from '@/academic/lib/i18n/languages';
import { translator } from '@/academic/lib/i18n/ui';
...
export function CourseChat({
  ...
  async function ask(text: string) {
    ...
    setBusy(true);
    try {
      const response = await fetch(`/api/studio/courses/${courseId}/ask`, {
        method: 'POST',
  ...
  return (
    <div className="flex h-full flex-col">
      <div ref={box} className={`flex-1 space-y-4 overflow-y-auto ${compact ? 'max-h-96' : ''}`}>
        {turns.length === 0 && (
          <div className="rounded-lg border border-studio-page-line bg-studio-page-card px-5 py-4">
            <div className="flex items-center gap-2 text-sm font-medium">
              <Bot size={16} className="text-studio-brand" />
            ...
            <div className="mt-3 flex flex-wrap gap-2">
              {suggestions.map((s) => (
                <button
                  ...
                  className="rounded-full border border-studio-page-line bg-white px-3 py-1.5 text-xs text-studio-ink-soft hover:border-studio-brand/40 hover:text-studio-brand-dark"
                >
                  {s}
                </button>
              ))}
        ...
        {turns.map((turn, i) => (
          turn.role === 'student' ? (
            <p key={i} className="ms-auto max-w-[80%] rounded-lg bg-studio-brand px-4 py-2.5 text-sm text-white">
              {turn.body}
            </p>
          ) : (
            <div
              ...
              className={`max-w-[95%] rounded-lg border px-5 py-4 ${
                turn.outsideCourse ? 'border-dashed border-studio-ink-faint bg-white'
                  : turn.refused ? 'border-amber-200 bg-amber-50'
                    : 'border-studio-page-line bg-studio-page-card'
              }`}
            >
              {/* THE TWO KNOWLEDGE SOURCES ARE NEVER MIXED ON A PAGE. A
                  ...
                  one of the two. */}
              {turn.outsideCourse && (
                <div className="mb-3 rounded border border-studio-ink-faint/40 bg-studio-page px-3 py-2">
                  <p className="text-[11px] font-semibold uppercase tracking-wide text-studio-ink-soft">
              ...
              {turn.answeredIn === 'this-lecture' && (
                <p className="mb-2 text-[11px] uppercase tracking-wide text-studio-ok">From this lecture</p>
              )}
              ...
              {turn.citations && turn.citations.length > 0 && (
                <div className="mt-3 border-t border-studio-page-line pt-3">
                  <p className="text-[11px] uppercase tracking-wide text-studio-ink-faint">{t('ai.fromYourLectures')}</p>
                  <ul className="mt-1 space-y-1">
                    {[...new Map(turn.citations.map((c) => [c.lectureSequence, c])).values()].map((c) => (
                      <li key={c.lectureSequence} className="text-xs text-studio-ink-soft">
                        Lecture {String(c.lectureSequence).padStart(2, '0')} — {c.lectureTitle}
                        <span className="text-studio-ink-faint"> · {c.artefactKind.replace(/_/g, ' ')}</span>
                      </li>
                    ))}
                  </ul>
                </div>
        ...
        {busy && (
          <p className="flex items-center gap-2 text-sm text-studio-ink-faint">
      ...
      {[courseLanguage, ...offeredLanguages].length > 1 && (
        <div className="mt-3 flex flex-wrap items-center gap-2">
          <Globe size={14} className="text-studio-ink-faint" />
            ...
            .map((code) => (
              <button
                ...
                className={`rounded-full border px-2.5 py-1 text-xs ${
                  language === code ? 'border-studio-brand bg-studio-brand text-white' : 'border-studio-page-line text-studio-ink-soft'
                }`}
              >
                {LANGUAGE_BY_CODE[code]?.endonym ?? code}
                {code === courseLanguage && (
                  <span className={`ms-1 text-[10px] uppercase ${language === code ? 'text-white/70' : 'text-studio-ink-faint'}`}>
                    taught in
                  </span>
                )}
              </button>
      ...
      >
        <input
          ...
          className="flex-1 rounded-md border border-studio-page-line bg-white px-3.5 py-2.5 text-sm"
        />
        <button
          type="submit" disabled={busy}
          className="inline-flex items-center gap-1.5 rounded-md bg-studio-brand px-3.5 py-2.5 text-sm font-medium text-white disabled:opacity-50"
        >
