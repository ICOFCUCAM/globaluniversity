...
import type { LectureProgress } from '@/academic/lib/study/progress';
  ...
  const item = (
    ...
    <Link
      ...
      className={`inline-flex items-center gap-1.5 rounded-full border px-3 py-1.5 text-xs ${
        finished ? 'border-emerald-200 bg-emerald-50 text-studio-ok' : 'border-studio-page-line text-studio-ink-soft hover:border-studio-brand/40'
      }`}
    >
  ...
  return (
    <div className="mt-3 flex flex-wrap gap-2">
      {has.notes && item('read', t('lecture.read'), BookOpen,
        `/academic-studio/lectures/${lectureId}#structured_notes`, !!done?.read, () => void record('read'))}
      {has.audio && item('listen', t('lecture.listen'), Headphones,
        `/academic-studio/lectures/${lectureId}#audio_15min`, !!done?.listened, () => void record('listened'))}
      {has.revision && item('revise', t('lecture.revise'), Brain,
        `/academic-studio/lectures/${lectureId}#revision_materials`, !!done?.revised, () => void record('revised'))}
      {item('quiz', done?.bestScore
        ? `Quiz — best ${done.bestScore.score}/${done.bestScore.outOf}`
        : t('lecture.quiz'), ListChecks, `/academic-studio/courses/${courseId}/study`, !!done?.quizTaken)}
      {item('ask', t('lecture.ask'), Bot, `/academic-studio/lectures/${lectureId}#ask`, false)}
    </div>
  );
}
