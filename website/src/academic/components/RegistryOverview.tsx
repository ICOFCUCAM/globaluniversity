...
import type { Artefact, Course, Department, Enrolment, Faculty, Lecture, Person, University } from '@/academic/lib/domain/types';
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-6">
        {stat.map((s) => (
          <Card key={s.label} className="px-4 py-3">
            <p className="text-[11px] uppercase tracking-wide text-studio-ink-faint">{s.label}</p>
        ...
        <Card className="px-5 py-4">
          ...
          <dl className="mt-3 space-y-2 text-sm">
            <div className="flex justify-between">
              <dt className="text-studio-ink-soft">Lectures recorded</dt>
              <dd className="font-medium">{lectures.length}</dd>
            </div>
            <div className="flex justify-between">
              <dt className="text-studio-ink-soft">Lectures processed</dt>
              <dd className="font-medium">{processed.size}</dd>
            </div>
            <div className="flex justify-between">
              <dt className="text-studio-ink-soft">Artefacts published</dt>
              <dd className="font-medium">{published.length}</dd>
            </div>
            <div className="flex justify-between">
              <dt className="text-studio-ink-soft">Awaiting a lecturer’s review</dt>
              <dd className="font-medium text-studio-warn">{awaiting.length}</dd>
            </div>
          </dl>
              ...
              <div key={faculty.id}>
                ...
                {departments.filter((d) => d.facultyId === faculty.id).map((department) => (
                  <div key={department.id} className="ms-4 mt-1">
                    <p className="text-sm text-studio-ink-soft">{department.name}</p>
                      ...
                      {courses.filter((c) => c.departmentId === department.id).map((course) => {
                        ...
                        return (
                          <li key={course.id} className="text-sm">
                            <Link href={`/academic-studio/courses/${course.id}`} className="text-studio-brand hover:underline">
                              {course.code}
                            </Link>
                            <span className="text-studio-ink-soft"> — {course.title}</span>
