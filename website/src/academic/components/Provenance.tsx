...
export function Provenance({ artefactId }: { artefactId: string }) {
  ...
  async function load() {
    ...
    try {
      const response = await fetch(`/api/studio/artefacts/${artefactId}/provenance`);
  ...
  return (
    <div className="mt-3">
      <button
        ...
        className="inline-flex items-center gap-1.5 text-xs text-studio-ink-soft hover:text-studio-brand-dark"
      >
        <GitBranch size={13} />
      ...
      {open && (
        <div className="mt-2 rounded-md border border-studio-page-line bg-studio-page px-4 py-3">
          {error && <p className="text-xs text-studio-bad">{error}</p>}
          {!chain && !error && (
            <p className="flex items-center gap-2 text-xs text-studio-ink-soft">
            ...
            <ol className="space-y-2">
              {chain.map((step, i) => (
                <li key={step.artefactId} className="flex gap-3 text-xs">
                  <span className="w-4 shrink-0 text-studio-ink-faint">{i + 1}</span>
                  <span>
                    <strong className="font-medium">{step.kind.replace(/_/g, ' ')}</strong>
                    <span className="text-studio-ink-faint"> · v{step.version}</span>
                    <span className="text-studio-ink-faint">
                      {' · '}{LANGUAGE_BY_CODE[step.language]?.name ?? step.language}
                    </span>
                    {step.translationOf && <span className="text-studio-ink-faint"> · translation</span>}
                    <span className="block text-studio-ink-soft">
                    ...
                    {step.staleSince && (
                      <span className="block text-studio-warn">
