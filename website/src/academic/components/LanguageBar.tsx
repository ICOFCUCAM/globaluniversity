...
import { LANGUAGES, LANGUAGE_BY_CODE, STANDING_NOTE, type TranslationStanding } from '@/academic/lib/i18n/languages';
...
export function LockedLanguage({ code }: { code: string }) {
  ...
  return (
    <div className="flex flex-wrap items-center gap-2 border-b border-studio-page-line bg-studio-page-card px-6 py-3 md:px-8">
      <Globe size={15} className="text-studio-ink-faint" />
      <span className="rounded-full border border-studio-brand bg-studio-brand px-3 py-1.5 text-xs text-white">
      ...
      <span className="text-xs text-studio-ink-faint">
        ...
        <a href="/academic-studio/profile" className="text-studio-brand hover:underline">your learning profile</a>{' '}
...
export function LanguageBar({
  ...
  return (
    <div className="flex flex-wrap items-center gap-2 border-b border-studio-page-line bg-studio-page-card px-6 py-3 md:px-8">
        ...
        return (
          <button
            ...
            className={`inline-flex items-center gap-1.5 rounded-full border px-3 py-1.5 text-xs transition ${
              active ? 'border-studio-brand bg-studio-brand text-white'
                : state?.exists || isMaster ? 'border-studio-page-line bg-white text-studio-ink-soft hover:border-studio-brand/40'
                  : 'border-dashed border-studio-page-line bg-studio-page text-studio-ink-faint hover:border-studio-brand/40'
            }`}
          ...
          >
            ...
            {isMaster ? (
              <span className={`text-[10px] uppercase tracking-wide ${active ? 'text-white/80' : 'text-studio-ink-faint'}`}>
                master
              </span>
            ) : state?.exists ? (
              state.standing === 'reviewed' ? <ShieldCheck size={12} className={active ? '' : 'text-studio-ok'} />
                : state.standing === 'stale' ? <TriangleAlert size={12} className={active ? '' : 'text-studio-bad'} />
                  : <span className={`text-[10px] ${active ? 'text-white/80' : 'text-studio-ink-faint'}`}>unread</span>
            ) : (
              <Languages size={12} />
            )}
          </button>
      ...
      {addable.length > 0 && (
        <select
          className="rounded-full border border-dashed border-studio-page-line bg-studio-page px-3 py-1.5 text-xs text-studio-ink-soft"
          value=""
...
export function StandingNote({
  ...
  return (
    <div className={`rounded-md border px-4 py-3 text-sm text-studio-ink-soft ${tone}`}>
      <p className="flex items-center gap-1.5">
        {standing === 'reviewed' ? <ShieldCheck size={14} className="text-studio-ok" />
          : <TriangleAlert size={14} className={standing === 'stale' ? 'text-studio-bad' : 'text-studio-warn'} />}
        {STANDING_NOTE[standing]}
      </p>
      <button
        type="button" onClick={onReadOriginal}
        className="mt-1.5 text-xs font-medium text-studio-brand hover:underline"
      >
