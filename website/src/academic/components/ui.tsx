...
import type { ArtefactState } from '@/academic/lib/domain/types';
...
export function PageHeader({
  ...
  return (
    <header className="border-b border-studio-page-line bg-studio-page-card px-6 py-5 md:px-8">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div className="min-w-0" {...own}>
          {eyebrow && <p className="text-[11px] uppercase tracking-wide text-studio-ink-faint">{eyebrow}</p>}
          <h1 className="text-xl font-semibold tracking-tight md:text-2xl">{title}</h1>
          {subtitle && <p className="mt-1 max-w-2xl text-sm text-studio-ink-soft">{subtitle}</p>}
        </div>
...
const STATE_STYLE: Record<ArtefactState, string> = {
  absent: 'bg-studio-page text-studio-ink-faint border-studio-page-line',
  queued: 'bg-studio-page text-studio-ink-soft border-studio-page-line',
  running: 'bg-studio-brand-tint text-studio-brand-dark border-studio-brand/30',
  ready: 'bg-amber-50 text-studio-warn border-amber-200',
  approved: 'bg-emerald-50 text-studio-ok border-emerald-200',
  published: 'bg-emerald-600 text-white border-emerald-600',
  failed: 'bg-red-50 text-studio-bad border-red-200',
};
...
export function Card({
  ...
  return (
    <section dir={dir} lang={lang} className={`rounded-lg border border-studio-page-line bg-studio-page-card ${className}`}>
      {children}
    </section>
  );
}
...
export function Empty({ title, body }: { title: string; body: string }) {
  return (
    <div className="rounded-lg border border-dashed border-studio-page-line bg-studio-page-card px-6 py-10 text-center">
      <p className="font-medium">{title}</p>
      <p className="mx-auto mt-1 max-w-md text-sm text-studio-ink-soft">{body}</p>
    </div>
  );
}
