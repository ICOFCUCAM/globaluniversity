...
import type { Passage } from '@/academic/lib/ai/tutor';
import { Empty } from '@/academic/components/ui';
    ...
    <div className="max-w-3xl">
      <form onSubmit={run} className="flex flex-wrap items-center gap-2">
        <input
          ...
          className="min-w-[16rem] flex-1 rounded-md border border-studio-page-line bg-white px-3.5 py-2.5 text-sm"
        />
        <select
          value={widenTo}
          onChange={(event) => setWidenTo(event.target.value as typeof widenTo)}
          className="rounded-md border border-studio-page-line px-2.5 py-2.5 text-xs text-studio-ink-soft"
        >
      ...
      <div className="mt-6 space-y-3">
        {results === null ? null : results.length === 0 ? (
          ...
          <article key={i} className="rounded-lg border border-studio-page-line bg-studio-page-card px-5 py-4">
            <p className="text-[11px] uppercase tracking-wide text-studio-ink-faint">
              {passage.courseCode && passage.courseCode !== courseCode && (
                <span className="me-1 rounded bg-amber-100 px-1.5 py-0.5 text-studio-warn">
            ...
            <Link
              href={`/academic-studio/lectures/${passage.lectureId}`}
              className="mt-2 inline-block text-xs font-medium text-studio-brand hover:underline"
            >
              Open the lecture
            </Link>
          </article>
