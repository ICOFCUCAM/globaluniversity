...
import type { Assignment, Reading, Submission } from '@/academic/lib/domain/types';
import { Card, Empty } from '@/academic/components/ui';
...
export function Coursework({
  ...
  async function send(body: Record<string, unknown>) {
    ...
    try {
      const response = await fetch('/api/studio/coursework', {
        method: 'POST',
  ...
  return (
    <div className="grid gap-6 lg:grid-cols-2">
      {error && <p className="lg:col-span-2 rounded border border-red-200 bg-red-50 px-3 py-2 text-sm text-studio-bad">{error}</p>}

      <section className="space-y-3">
        <h2 className="text-sm font-semibold uppercase tracking-wide text-studio-ink-faint">Reading</h2>
        {readings.length === 0 ? (
          ...
          <Card className="px-5 py-4">
            ...
            >
              <input name="citation" required placeholder="The citation, as you would write it"
                className="w-full rounded border border-studio-page-line px-3 py-2 text-sm" />
              <input name="url" placeholder="A link, if there is one"
                className="w-full rounded border border-studio-page-line px-3 py-2 text-sm" />
              <input name="note" placeholder="Why you set it"
                className="w-full rounded border border-studio-page-line px-3 py-2 text-sm" />
              <div className="flex items-center gap-3">
                <select name="kind" className="rounded border border-studio-page-line px-2 py-1.5 text-sm">
                ...
                <label className="flex items-center gap-1.5 text-xs text-studio-ink-soft">
                  <input type="checkbox" name="required" defaultChecked /> Essential
                </label>
                <button type="submit" disabled={busy}
                  className="ms-auto rounded bg-studio-brand px-3 py-1.5 text-xs font-medium text-white">
                  Add
                </button>
              </div>
            </form>
      ...
      <section className="space-y-3">
        <h2 className="text-sm font-semibold uppercase tracking-wide text-studio-ink-faint">Assignments</h2>
        {assignments.length === 0 ? (
          ...
          return (
            <Card key={assignment.id} className="px-5 py-4">
              ...
              <p className="mt-1 text-sm text-studio-ink-soft">{assignment.brief}</p>
                ...
                <div className="mt-3 space-y-2">
                  {!assignment.published && (
                    <button type="button" disabled={busy}
                      ...
                      className="rounded border border-studio-page-line px-2.5 py-1 text-xs text-studio-ink-soft">
                      Set it
                    </button>
                  )}
                  <p className="text-xs text-studio-ink-faint">{list.length} handed in</p>
                  {list.map((submission) => {
                    ...
                    return (
                      <div key={submission.id} className="rounded border border-studio-page-line px-3 py-2">
                        <p className="text-xs font-medium">
                          {who}
                          {submission.late ? ' · late' : ''}
                          {submission.markedAt ? ` · marked${submission.returnedAt ? ' and returned' : ', not returned'}` : ''}
                        </p>
                        <p className="mt-1 whitespace-pre-wrap text-sm text-studio-ink-soft">{submission.body}</p>
                        <div className="mt-2 flex flex-wrap items-center gap-2">
                          <input
                            type="number" placeholder="Mark" defaultValue={submission.mark}
                            className="w-20 rounded border border-studio-page-line px-2 py-1 text-xs"
                          ...
                          <input
                            ...
                            className="min-w-[16rem] flex-1 rounded border border-studio-page-line px-2 py-1 text-xs"
                          ...
                          <button
                            ...
                            className="rounded bg-studio-brand px-3 py-1.5 text-xs font-medium text-white"
                          >
                            Mark
                          </button>
                        </div>
                  ...
                  {list.some((s) => s.markedAt && !s.returnedAt) && (
                    <button
                      ...
                      className="rounded border border-emerald-300 bg-emerald-50 px-3 py-1.5 text-xs font-medium text-studio-ok"
                    >
                ...
                <div className="mt-3">
                  ...
                  {!mine?.markedAt && (
                    <button
                      ...
                      className="mt-2 inline-flex items-center gap-1.5 rounded bg-studio-brand px-3.5 py-2 text-sm font-medium text-white"
                    >
            ...
            >
              ...
              <div className="flex flex-wrap items-center gap-2">
                <input name="marksOutOf" type="number" placeholder="Out of"
                  className="w-24 rounded border border-studio-page-line px-2 py-1.5 text-sm" />
                <input name="dueAt" type="date"
                  className="rounded border border-studio-page-line px-2 py-1.5 text-sm" />
                <button type="submit" disabled={busy}
                  className="ms-auto rounded bg-studio-brand px-3 py-1.5 text-xs font-medium text-white">
                  Set it
                </button>
