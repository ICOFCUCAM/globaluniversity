...
import { mark, parseQuiz, type Marked } from '@/academic/lib/study/quiz';
...
export function QuizRunner({
  ...
  if (!quiz.questions.length) {
    return (
      <div className="rounded-lg border border-studio-page-line bg-studio-page-card p-6">
  ...
  return (
    <div className="rounded-lg border border-studio-page-line bg-studio-page-card p-6" dir={dir}>
      ...
      {unreviewed && (
        <p className="mt-1 text-xs text-studio-warn">
          ...
          return (
            <li key={question.n}>
              ...
              {question.options.length > 0 ? (
                        ...
                        <label className={`flex items-start gap-2 rounded border px-3 py-2 text-sm ${
                          isAnswer ? 'border-emerald-300 bg-emerald-50'
                            : chosen && marking?.right === false ? 'border-red-300 bg-red-50'
                              : chosen ? 'border-studio-brand bg-studio-brand-tint' : 'border-studio-page-line'
                        }`}>
                          <input
                          ...
                          {isAnswer && <Check size={14} className="ms-auto mt-0.5 shrink-0 text-studio-ok" />}
                          {chosen && marking?.right === false && <X size={14} className="ms-auto mt-0.5 shrink-0 text-studio-bad" />}
                        </label>
                      </li>
                    );
                  })}
                ...
                <textarea
                  className="mt-2 w-full rounded border border-studio-page-line px-3 py-2 text-sm"
                  rows={2}
                  disabled={!!result}
              ...
              {result && question.answer && (
                <div className="mt-2 rounded border border-studio-page-line bg-studio-page px-3 py-2 text-sm">
                  <p><strong>Answer:</strong> {question.answer}</p>
                  {question.explanation && <p className="mt-0.5 text-studio-ink-soft">{question.explanation}</p>}
                  {marking?.right === null && (
      ...
      {!result && (
        <button
          ...
          className="mt-5 inline-flex items-center gap-2 rounded-md bg-studio-brand px-4 py-2 text-sm font-medium text-white disabled:opacity-50"
        >
      ...
      {quiz.remainder && (
        <details className="mt-4 text-xs text-studio-ink-faint">
