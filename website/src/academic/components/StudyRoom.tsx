...
import { QuizRunner } from '@/academic/components/QuizRunner';
import { FlashcardDeck } from '@/academic/components/FlashcardDeck';
...
export function StudyRoom({
  ...
  async function make(kind: 'test' | 'flashcards' | 'audio_revision') {
    ...
    try {
      const response = await fetch(`/api/studio/courses/${courseId}/study-aid`, {
        method: 'POST',
              ...
              return (
                <button
                  ...
                  className={`rounded border px-2 py-1 text-[11px] ${
                    chosen ? 'border-studio-brand bg-studio-brand text-white' : 'border-studio-page-line text-studio-ink-soft'
                  }`}
                  title={lecture.title}
                >
          ...
          <div className="mt-3 space-y-1.5">
            {([
              ...
              <button
                ...
                className="flex w-full items-center gap-2 rounded border border-studio-page-line bg-white px-3 py-2 text-xs text-studio-ink-soft hover:border-studio-brand/40 disabled:opacity-40"
              >
                ...
                <li key={aid.id}>
                  <button
                    ...
                    className={`w-full rounded px-2 py-1.5 text-start text-xs ${
                      open === aid.id ? 'bg-studio-brand-tint text-studio-brand-dark' : 'text-studio-ink-soft hover:bg-studio-page'
                    }`}
                  >
                    {aid.title}
                    {aid.unreviewed && <span className="block text-[10px] text-studio-ink-faint">{t('study.notReviewed')}</span>}
                  </button>
                </li>
              ))}
            </ul>
