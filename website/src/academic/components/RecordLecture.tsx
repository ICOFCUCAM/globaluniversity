...
export function RecordLecture({ onRecorded }: { onRecorded: (file: File) => void }) {
  ...
  return (
    <div className="rounded-md border border-studio-page-line px-3 py-2">
      <div className="flex flex-wrap items-center gap-3">
        {state === 'recording' ? (
          <button
            type="button" onClick={stop}
            className="inline-flex items-center gap-2 rounded bg-studio-bad px-3 py-1.5 text-xs font-medium text-white"
          >
          ...
          <button
            ...
            className="inline-flex items-center gap-2 rounded border border-studio-page-line px-3 py-1.5 text-xs text-studio-ink-soft hover:border-studio-brand/40"
          >
            {state === 'saving' ? <Loader2 size={12} className="animate-spin" /> : <Circle size={12} className="text-studio-bad" />}
            Record in the room
          </button>
        )}
        {state === 'recording' && (
      ...
      {error && <p className="mt-1 text-xs text-studio-bad">{error}</p>}
    </div>
  );
}
