...
import { LANGUAGE_BY_CODE, direction } from '@/academic/lib/i18n/languages';
...
export function LiveRoom({
  ...
  const follow = useCallback(async () => {
    ...
    try {
      const response = await fetch(`/api/studio/courses/${courseId}/live?session=${session.id}`);
  ...
  async function post(body: Record<string, unknown>) {
    ...
    try {
      const response = await fetch(`/api/studio/courses/${courseId}/live`, {
        method: 'POST',
    ...
    return (
      <Card className="px-5 py-6">
        <h2 className="flex items-center gap-2 text-sm font-semibold">
          <Radio size={16} className="text-studio-brand" /> No lecture is being given
        </h2>
        ...
        {teaching && (
          ...
          >
            <input
              ...
              className="min-w-[18rem] flex-1 rounded-md border border-studio-page-line px-3 py-2 text-sm"
            />
            <button
        ...
        {error && <p className="mt-2 text-xs text-studio-bad">{error}</p>}
      </Card>
    );
  }
  ...
  return (
    <div className="space-y-4">
      {!engine.live && (
        <div className="rounded-md border border-amber-200 bg-amber-50 px-4 py-2.5 text-xs text-studio-ink-soft">
          <span className="font-semibold uppercase tracking-wide text-studio-warn">Not a translation</span>
        ...
        <Card className="px-5 py-4">
          ...
          >
            <input
              ...
              className="min-w-[20rem] flex-1 rounded-md border border-studio-page-line px-3 py-2 text-sm"
            />
            <button
              type="submit" disabled={busy}
              className="rounded-md bg-studio-brand px-3.5 py-2 text-sm font-medium text-white hover:bg-studio-brand-dark disabled:opacity-50"
            ...
            <button
              ...
              className="inline-flex items-center gap-1.5 rounded-md border border-studio-page-line px-3.5 py-2 text-sm text-studio-ink-soft"
            >
      ...
      <Card className="px-5 py-4" dir={direction(language)}>
        {heard.length === 0 ? (
          <p className="text-sm text-studio-ink-faint">Nothing has been said yet.</p>
        ) : (
          <ol className="space-y-3">
            {heard.map((line) => (
              <li key={line.sequence} className="text-sm">
                ...
                {line.source === 'floor' && (
                  <span>
                    <span className="text-studio-ink-soft">{line.text}</span>
                    <span className="mt-1 flex items-center gap-1.5 text-[11px] text-studio-warn">
                ...
                {(line.source === 'silence' || line.source === 'notice') && (
                  <span className="flex items-center gap-1.5 text-[11px] text-studio-warn">
                    <ShieldAlert size={12} /> {line.because}
                  </span>
                )}
              </li>
