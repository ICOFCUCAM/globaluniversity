...
import { LANGUAGE_BY_CODE } from '@/academic/lib/i18n/languages';
import { PLATFORM_VOICES, SPEEDS } from '@/academic/lib/voice/voices';
import { ACCESSIBILITY_CHOICES, settingsOf } from '@/academic/lib/access/accessibility';
import { fill, translator } from '@/academic/lib/i18n/ui';
...
export function LearningProfile({ person, isStudent }: { person: Person; isStudent: boolean }) {
  ...
  return (
    <div className="grid gap-4 lg:grid-cols-2">
      {error && (
        <p className="lg:col-span-2 rounded border border-red-200 bg-red-50 px-3 py-2 text-sm text-studio-bad">{error}</p>
      )}

      <Card className="px-5 py-4">
        <h2 className="flex items-center gap-2 text-sm font-semibold">
          <Globe size={16} className="text-studio-brand" /> {t('profile.workingLanguage')}
        </h2>
        <p className="mt-2 text-2xl font-semibold">
          {language ? LANGUAGE_BY_CODE[language]?.endonym ?? language : 'Not set'}
        </p>
        <p className="mt-1 flex items-center gap-1.5 text-xs text-studio-ink-faint">
          <Lock size={12} />
          {isStudent
        ...
        {lastChange && (
          <p className="mt-2 rounded border border-studio-page-line bg-studio-page px-3 py-2 text-xs text-studio-ink-soft">
      ...
      <Card className="px-5 py-4">
        <h2 className="flex items-center gap-2 text-sm font-semibold">
          <Headphones size={16} className="text-studio-brand" /> {t('profile.listening')}
        </h2>
        <p className="mt-1 text-xs text-studio-ink-soft">
          {t('profile.voiceIsYours')}
        </p>

        <label className="mt-3 block text-[11px] uppercase tracking-wide text-studio-ink-faint">{t('profile.preferredVoice')}</label>
        <select
        ...
        <label className="mt-3 block text-[11px] uppercase tracking-wide text-studio-ink-faint">{t('profile.speed')}</label>
            ...
            <button
              ...
              className={`rounded border px-2.5 py-1 text-xs ${
                (person.audioSpeed ?? 1) === speed
                  ? 'border-studio-brand bg-studio-brand text-white' : 'border-studio-page-line text-studio-ink-soft'
              }`}
            >
              {speed}×
            </button>
      ...
      <Card className="px-5 py-4 lg:col-span-2">
        <h2 className="flex items-center gap-2 text-sm font-semibold">
          <Accessibility size={16} className="text-studio-brand" /> {t('profile.reading')}
        </h2>
        ...
        <div className="mt-4 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {ACCESSIBILITY_CHOICES.map((choice) => {
            ...
            return (
              <div key={choice.key}>
                <p className="text-[11px] uppercase tracking-wide text-studio-ink-faint">{t(choice.label)}</p>
                ...
                <p className="mt-1.5 text-xs text-studio-ink-faint">{t(choice.blurb)}</p>
              </div>
            );
          })}
        </div>
        ...
        <p className="mt-4 rounded border border-studio-page-line bg-studio-page px-3 py-2 text-xs text-studio-ink-soft">
      ...
      {!isStudent && (
        <Card className="px-5 py-4 lg:col-span-2">
          <h2 className="flex items-center gap-2 text-sm font-semibold">
            <Mic size={16} className="text-studio-brand" /> Your voice
          </h2>
          <p className="mt-1 text-sm text-studio-ink-soft">
          ...
          {person.voiceConsent && !person.voiceConsent.revokedAt ? (
            <div className="mt-3 rounded border border-emerald-200 bg-emerald-50 px-4 py-3">
              <p className="flex items-center gap-1.5 text-sm text-studio-ok">
                <ShieldCheck size={14} />
              ...
              <button
                ...
                className="mt-2 rounded border border-studio-page-line bg-white px-3 py-1.5 text-xs text-studio-ink-soft"
              >
                Withdraw permission
              </button>
            </div>
            ...
            <div className="mt-3 flex flex-wrap gap-2">
              <button
                ...
                className="rounded-md border border-studio-page-line bg-white px-3.5 py-2 text-sm text-studio-ink-soft hover:border-studio-brand/40"
              >
              ...
              {person.voiceConsent?.revokedAt && (
                <p className="w-full text-xs text-studio-ink-faint">
