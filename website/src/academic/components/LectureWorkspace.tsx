...
import { Markdown } from '@/academic/components/Markdown';
import { Provenance } from '@/academic/components/Provenance';
import { WordCheck } from '@/academic/components/WordCheck';
import { StateBadge } from '@/academic/components/ui';
import { CopyButton, LanguageBar, LockedLanguage, StandingNote, type LanguageRow } from '@/academic/components/LanguageBar';
import { ListeningPanel, type VoiceOfferView } from '@/academic/components/ListeningPanel';
import { AudioLesson } from '@/academic/components/AudioLesson';
import { clock, direction, estimateSeconds, LANGUAGE_BY_CODE, TRANSLATABLE } from '@/academic/lib/i18n/languages';
...
export function LectureWorkspace({
  ...
  async function run(kind: ArtefactKind, regenerate = false) {
    ...
    try {
      const response = await fetch(`/api/studio/lectures/${lecture.id}/run`, {
        method: 'POST',
  ...
  async function translateKind(kind: ArtefactKind, code: string) {
    ...
    try {
      const response = await fetch(`/api/studio/artefacts/${source.id}/translate`, {
        method: 'POST',
        headers: { 'content-type': 'application/json' },
  ...
  async function vouch(artefact: Artefact) {
    ...
    try {
      const response = await fetch(`/api/studio/artefacts/${artefact.id}/translate`, {
        method: 'POST',
        headers: { 'content-type': 'application/json' },
  ...
  async function act(artefact: Artefact, action: string, body?: string) {
    setBusy(artefact.kind); setError(null);
    try {
      const response = await fetch(`/api/studio/artefacts/${artefact.id}`, {
        method: 'POST',
        headers: { 'content-type': 'application/json' },
  ...
  return (
    ...
    <div className="grid gap-6 px-6 py-6 md:px-8 lg:grid-cols-[260px_1fr]">
      ...
      <aside className="space-y-2">
        <h2 className="text-sm font-semibold uppercase tracking-wide text-studio-ink-faint">Pipeline</h2>
        {translated && (
          <p className="rounded-md border border-studio-page-line bg-studio-page px-3 py-2 text-[11px] text-studio-ink-soft">
          ...
          return (
            <div
              ...
              className={`rounded-md border px-3 py-2.5 ${
                open === stage.kind ? 'border-studio-brand/50 bg-studio-brand-tint' : 'border-studio-page-line bg-studio-page-card'
              }`}
            >
              <button
                type="button"
              ...
              >
                <div className="flex items-center justify-between gap-2">
                  <span className={`text-sm ${artefact ? 'font-medium' : 'text-studio-ink-faint'}`}>{stage.label}</span>
                  <StateBadge state={state} />
                </div>
                {artefact?.staleSince && (
                  <p className="mt-1 text-[11px] text-studio-warn">
              ...
              {translated && canTranslate && (
                <button
                  ...
                  className="mt-2 inline-flex items-center gap-1.5 rounded border border-studio-page-line bg-white px-2 py-1 text-[11px] text-studio-ink-soft disabled:opacity-40 hover:border-studio-brand/40"
        ...
        {canEdit && !translated && (
          <div className="rounded-md border border-studio-page-line bg-studio-page-card px-3 py-3 space-y-3">
            <div>
              <label className="block text-[11px] uppercase tracking-wide text-studio-ink-faint">Audio mode</label>
              <select
                className="mt-1 w-full rounded border border-studio-page-line px-2 py-1 text-xs"
            ...
            <div>
              <label className="block text-[11px] uppercase tracking-wide text-studio-ink-faint">Teaching voice</label>
              <select
                className="mt-1 w-full rounded border border-studio-page-line px-2 py-1 text-xs"
            ...
            <div>
              <label className="block text-[11px] uppercase tracking-wide text-studio-ink-faint">Revision set</label>
              <select
                className="mt-1 w-full rounded border border-studio-page-line px-2 py-1 text-xs"
      ...
      <div className="min-w-0 space-y-4">
        {error && (
          <div className="rounded-md border border-red-200 bg-red-50 px-4 py-3 text-sm text-studio-bad">{error}</div>
        )}

        {!shown ? (
          <p className="rounded-lg border border-dashed border-studio-page-line bg-studio-page-card px-6 py-10 text-center text-sm text-studio-ink-soft">
          ...
          <>
            <div className="flex flex-wrap items-center justify-between gap-3">
              <div>
                ...
                <p className="text-xs text-studio-ink-faint">
              ...
              <div className="flex flex-wrap items-center gap-2">
                ...
                {translated && canApproveTranslation && shown.translationStanding !== 'reviewed' && (
                  <button
                    ...
                    className="rounded border border-emerald-300 bg-emerald-50 px-2.5 py-1.5 text-xs font-medium text-studio-ok"
                  >
                ...
                {translated && master && (
                  <button
                    ...
                    className="rounded border border-studio-page-line px-2.5 py-1.5 text-xs text-studio-ink-soft hover:border-studio-brand/40"
                  >
                ...
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  {draft === null ? (
                    <button
                      ...
                      className="inline-flex items-center gap-1.5 rounded border border-studio-page-line px-2.5 py-1.5 text-xs text-studio-ink-soft hover:border-studio-brand/40"
                    >
                      <Pencil size={13} /> Edit
                    </button>
                  ) : (
                    <>
                      <button
                        ...
                        disabled={busy !== null}
                        className="inline-flex items-center gap-1.5 rounded bg-studio-brand px-2.5 py-1.5 text-xs font-medium text-white hover:bg-studio-brand-dark"
                      >
                        <Check size={13} /> Save correction
                      </button>
                      <button
                        type="button" onClick={() => setDraft(null)}
                        className="rounded border border-studio-page-line px-2.5 py-1.5 text-xs text-studio-ink-soft"
                      >
                        Cancel
                      </button>
                    </>
                  ...
                  {shown.kind === 'teaching_script' && (
                    <button
                      ...
                      className={`rounded border px-2.5 py-1.5 text-xs ${
                        shown.wordCheck
                          ? 'border-emerald-300 bg-emerald-50 text-studio-ok'
                          : 'border-amber-300 bg-amber-50 text-studio-warn'
                      }`}
                    >
                  ...
                  {shown.state === 'ready' && (
                    <button
                      ...
                      disabled={busy !== null}
                      className="inline-flex items-center gap-1.5 rounded border border-emerald-300 bg-emerald-50 px-2.5 py-1.5 text-xs font-medium text-studio-ok"
                    >
                  ...
                  {shown.state === 'published' && (
                    <button
                      ...
                      disabled={busy !== null}
                      className="inline-flex items-center gap-1.5 rounded border border-studio-page-line px-2.5 py-1.5 text-xs text-studio-ink-soft"
                    >
            ...
            {shown.origin !== 'lecturer' && !translated && (
              <div className="rounded-md border border-studio-page-line bg-studio-page px-4 py-2.5 text-xs text-studio-ink-soft">
                <span className="font-semibold uppercase tracking-wide text-studio-ink-faint">Lecture content</span>
            ...
            {translated && (
              <div className="rounded-md border border-studio-page-line bg-studio-page px-4 py-2.5 text-xs text-studio-ink-soft">
                <span className="font-semibold uppercase tracking-wide text-studio-ink-faint">Translation</span>
                {' — carried from the '}
            ...
            {shown.origin === 'lecturer' && shown.correctedByLecturer && (
              <div className="rounded-md border border-emerald-200 bg-emerald-50 px-4 py-2.5 text-xs text-studio-ink-soft">
                <span className="font-semibold uppercase tracking-wide text-studio-ok">Corrected by the lecturer</span>
            ...
            {shown.state === 'ready' && canEdit && (
              <div className="rounded-md border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-studio-ink-soft">
                <strong className="text-studio-warn">Not yet seen by anybody but you.</strong> Read it, correct
            ...
            {shown.translationFindings && shown.translationFindings.length > 0 && (
              <div className="rounded-md border border-studio-page-line bg-studio-page px-4 py-3 text-sm">
                <p className="font-medium">What the language-agnostic checks found</p>
                <ul className="mt-1 space-y-1 text-xs text-studio-ink-soft">
                  {shown.translationFindings.map((finding, i) => (
                    <li key={i}>
                      <span className={`font-medium uppercase tracking-wide ${
                        finding.severity === 'reject' ? 'text-studio-bad' : 'text-studio-warn'
                      }`}>
            ...
            {shown.terminology && shown.terminology.length > 0 && (
              <div className="rounded-md border border-red-200 bg-red-50 px-4 py-3 text-sm">
                <p className="font-medium text-studio-bad">
                ...
                <ul className="mt-2 space-y-1 text-xs text-studio-ink-soft">
                  {shown.terminology.map((finding, i) => (
                    <li key={i}>
                      <span className="font-medium uppercase tracking-wide text-studio-bad">{finding.kind}</span>
                      {' · '}{finding.note}
                    </li>
                  ))}
                </ul>
            ...
            {shown.verification && (
              <div className={`rounded-md border px-4 py-3 text-sm ${
                shown.verification.error ? 'border-studio-page-line bg-studio-page text-studio-ink-soft'
                  ...
                  <>
                    ...
                    {shown.verification.checks.filter((c) => c.status !== 'preserved').map((c, i) => (
                      <div key={i} className="mt-2 border-s-2 border-amber-300 ps-3 text-xs">
                        <p className="font-medium uppercase tracking-wide text-studio-warn">{c.status}</p>
                        {c.original && <p className="text-studio-ink-soft">Lecture: “{c.original}”</p>}
                        {c.output && <p className="text-studio-ink-soft">Now: “{c.output}”</p>}
                        {c.note && <p className="text-studio-ink-faint">{c.note}</p>}
                      </div>
                    ))}
                  </>
                )}
            ...
            {shown.error && (
              <div className="rounded-md border border-red-200 bg-red-50 px-4 py-3 text-sm text-studio-bad">
                {shown.error}
              </div>
            )}
            ...
            {compare && sourceArtefact?.body ? (
              <div className="grid gap-4 lg:grid-cols-2">
                <div className="rounded-lg border border-studio-page-line bg-studio-page-card p-5">
                  <p className="mb-2 text-[11px] uppercase tracking-wide text-studio-ink-faint">As transcribed</p>
                  <div className="prose-academic whitespace-pre-wrap text-studio-ink-soft">{sourceArtefact.body}</div>
                </div>
                <div className="rounded-lg border border-studio-page-line bg-studio-page-card p-5">
                  <p className="mb-2 text-[11px] uppercase tracking-wide text-studio-ink-faint">Corrected</p>
                  <div className="prose-academic whitespace-pre-wrap">{shown.body}</div>
                </div>
              </div>
            ) : draft !== null ? (
              <textarea
                className="h-[32rem] w-full rounded-lg border border-studio-page-line bg-studio-page-card p-5 font-mono text-[13px] leading-6"
                value={draft}
                onChange={(event) => setDraft(event.target.value)}
              />
            ) : showMaster && master?.body ? (
              <div className="grid gap-4 lg:grid-cols-2">
                <article className="rounded-lg border border-studio-page-line bg-studio-page-card p-5">
                  <p className="mb-2 text-[11px] uppercase tracking-wide text-studio-ink-faint">
                    {LANGUAGE_BY_CODE[originalLanguage]?.name ?? originalLanguage} — master, what the lecturer taught
                  </p>
                  <Markdown source={master.body} />
                </article>
                <article className="rounded-lg border border-studio-page-line bg-studio-page-card p-5" dir={dir}>
                  <p className="mb-2 text-[11px] uppercase tracking-wide text-studio-ink-faint" dir="ltr">
                    {LANGUAGE_BY_CODE[language]?.name ?? language}
                  </p>
              ...
              <article className="rounded-lg border border-studio-page-line bg-studio-page-card p-6 md:p-8" dir={dir}>
                {shown.kind === 'audio_15min' ? (
                  shown.mediaPath || shown.parts?.some((p) => p.mediaPath) ? (
                    <AudioLesson
                      ...
                      parts={(shown.parts?.filter((p) => p.mediaPath) ?? [])
                        .map((p) => ({ label: p.label, src: `/api/studio/media/${p.mediaPath}`, seconds: p.seconds }))
                        .concat(shown.mediaPath && !shown.parts?.length
                          ? [{ label: '15-minute lesson', src: `/api/studio/media/${shown.mediaPath}`, seconds: shown.mediaSeconds }]
                          : [])}
                    />
                  ) : (
                  ...
                  <div className="space-y-2">
                    {shown.segments.map((segment, i) => (
                      <p key={i} className="flex gap-3 text-sm">
                        <span className="w-14 shrink-0 font-mono text-xs text-studio-ink-faint">
                          {Math.floor(segment.start / 60)}:{String(Math.floor(segment.start % 60)).padStart(2, '0')}
                        </span>
                        {segment.speaker && (
                          <span className="w-24 shrink-0 text-xs font-medium text-studio-ink-soft">{segment.speaker}</span>
                        )}
                  ...
                  <p className="text-sm text-studio-ink-faint">Nothing in this artefact yet.</p>
                )}
              </article>
            )}
