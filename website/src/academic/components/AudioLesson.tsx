  ...
  return (
    <section className="rounded-lg border border-studio-page-line bg-studio-page-card px-5 py-4">
      <h3 className="flex items-center gap-2 text-sm font-semibold">
        <Headphones size={16} className="text-studio-brand" /> {part.label}
      </h3>
      ...
      <div className="mt-3 flex flex-wrap items-center gap-2">
        {parts.length > 1 && parts.map((other, i) => (
          <button
            ...
            className={`rounded border px-2.5 py-1 text-xs ${
              i === at ? 'border-studio-brand bg-studio-brand text-white' : 'border-studio-page-line text-studio-ink-soft'
            }`}
          >
            Part {i + 1}
          </button>
        ...
        <div className="ms-auto flex items-center gap-1.5">
          ...
          <a
            ...
            className="ms-2 inline-flex items-center gap-1 rounded border border-studio-page-line px-2 py-1 text-[11px] text-studio-ink-soft"
          >
      ...
      {script && (captions ? (
        <div className="mt-4 border-t border-studio-page-line pt-4">
          <p className="mb-2 flex items-center gap-1.5 text-[11px] uppercase tracking-wide text-studio-ink-faint">
            <Captions size={12} /> What is said — the whole script, not timed to the audio
          </p>
          <Markdown source={script} />
        </div>
      ) : (
        <details className="mt-4 border-t border-studio-page-line pt-4">
          <summary className="cursor-pointer text-xs text-studio-ink-soft">
            Read what is said
          </summary>
