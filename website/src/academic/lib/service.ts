...
export async function runStage(
  ...
  await tell(
    ...
    stageLabel, `/academic-studio/lectures/${lecture.id}`,
  );
...
export async function translateArtefact(
  ...
  try {
    ...
    await tell(store, artefact.ownerId, 'translation-ready',
      ...
      `/academic-studio/lectures/${artefact.lectureId}`);
    await store.addVersion({
      id: randomUUID(),
      artefactId: artefact.id,
      version: artefact.version,
...
export async function setAssignment(
  ...
  if (saved.published && !existing?.published) {
    for (const enrolment of await store.enrolments(courseId)) {
      ...
      await tell(store, enrolment.studentId, 'work-set', saved.title, `/academic-studio/courses/${courseId}/work`);
    }
  }
  return saved;
}
...
export async function returnWork(
  ...
  for (const submission of submissions) {
    ...
    await tell(store, submission.studentId, 'work-returned', assignment.title,
      `/academic-studio/courses/${assignment.courseId}/work`);
    released += 1;
  }
  return released;
}
...
export async function issueCertificate(
  ...
  await tell(store, studentId, 'work-returned', `${where.course.code} — your certificate`, '/academic-studio/profile');
...
export async function closeLive(
  ...
  await tell(store, actor.id, 'awaiting-review',
    ...
    `/academic-studio/lectures/${lecture.id}`);
