...
import { cohortOn, knowledgeBase, myProgressOn } from '@/academic/lib/service';
import { mayEnterCourse } from '@/academic/lib/domain/ownership';
import { translate } from '@/academic/lib/i18n/ui';
import { TodaysLearning } from '@/academic/components/TodaysLearning';
    ...
    <div>
      <PageHeader
        ...
        actions={(
          <>
            <Link
              href={`/academic-studio/courses/${course.id}/live`}
            ...
            <Link
              href={`/academic-studio/courses/${course.id}/search`}
              className="inline-flex items-center gap-2 rounded-md border border-studio-page-line px-3.5 py-2 text-sm text-studio-ink-soft hover:border-studio-brand/40"
            >
              <Search size={16} /> Search
            </Link>
            <Link
              href={`/academic-studio/courses/${course.id}/work`}
              className="inline-flex items-center gap-2 rounded-md border border-studio-page-line px-3.5 py-2 text-sm text-studio-ink-soft hover:border-studio-brand/40"
            >
              <BookMarked size={16} /> Reading and work
            </Link>
            <Link
              href={`/academic-studio/courses/${course.id}/study`}
              className="inline-flex items-center gap-2 rounded-md border border-studio-page-line px-3.5 py-2 text-sm text-studio-ink-soft hover:border-studio-brand/40"
            >
              <ListChecks size={16} /> Revision
            </Link>
            <Link
              href={`/academic-studio/courses/${course.id}/ai`}
              className="inline-flex items-center gap-2 rounded-md border border-studio-brand/30 bg-studio-brand-tint px-3.5 py-2 text-sm font-medium text-studio-brand-dark hover:bg-studio-brand/10"
            >
              <Bot size={16} /> Course AI
            </Link>
            {teaching && (
              <Link
                href={`/academic-studio/courses/${course.id}/settings`}
                className="inline-flex items-center gap-2 rounded-md border border-studio-page-line px-3.5 py-2 text-sm text-studio-ink-soft hover:border-studio-brand/40"
              >
            ...
            {teaching && (
              <Link
                href={`/academic-studio/courses/${course.id}/new-lecture`}
                className="inline-flex items-center gap-2 rounded-md bg-studio-brand px-3.5 py-2 text-sm font-medium text-white hover:bg-studio-brand-dark"
              >
      ...
      <div className="grid gap-6 px-6 py-6 md:px-8 lg:grid-cols-3">
        <div className="space-y-3 lg:col-span-2">
          <h2 className="text-sm font-semibold uppercase tracking-wide text-studio-ink-faint">Lectures</h2>
          {lectures.length === 0 ? (
            ...
            return (
              <Card key={lecture.id} className="px-5 py-4">
                <div className="flex flex-wrap items-baseline justify-between gap-2">
                  <Link href={`/academic-studio/lectures/${lecture.id}`} className="font-medium hover:text-studio-brand">
                ...
                {lecture.abstract && <p className="mt-1 text-sm text-studio-ink-soft">{lecture.abstract}</p>}
                {student && (
                  <TodaysLearning
                    courseId={course.id}
                ...
                {cohort && (() => {
                  ...
                  return (
                    <p className={`mt-2 text-xs ${quiet ? 'text-studio-warn' : 'text-studio-ink-faint'}`}>
                ...
                <div className="mt-3 flex flex-wrap gap-2">
                  {visible.map((a) => {
                    ...
                    return (
                      <Link
                        key={a.id}
                        href={`/academic-studio/lectures/${lecture.id}#${a.kind}`}
                        className="inline-flex items-center gap-1.5 rounded border border-studio-page-line px-2 py-1 text-xs text-studio-ink-soft hover:border-studio-brand/40 hover:text-studio-brand-dark"
                      >
                        <Icon size={13} />
                        {a.kind.replace(/_/g, ' ')}
                        {a.state !== 'published' && !student && (
                          <span className="text-[10px] text-studio-ink-faint">· {a.state}</span>
                        )}
                      </Link>
                    );
                  })}
                  {!visible.length && (
                    <span className="text-xs text-studio-ink-faint">Nothing published from this lecture yet.</span>
                  )}
                </div>
              </Card>
            );
        ...
        <div className="space-y-4">
          <Card className="px-5 py-4">
            ...
            <p className="mt-1 text-xs text-studio-ink-soft">
            ...
            <dl className="mt-3 grid grid-cols-2 gap-3 text-sm">
              <div>
                <dt className="text-[11px] uppercase tracking-wide text-studio-ink-faint">Concepts</dt>
                <dd className="text-lg font-semibold">{knowledge.nodes.length}</dd>
              </div>
              <div>
                <dt className="text-[11px] uppercase tracking-wide text-studio-ink-faint">Defined</dt>
            ...
            {!student && knowledge.undefined.length > 0 && (
              <div className="mt-4 rounded border border-amber-200 bg-amber-50 px-3 py-2">
                <p className="text-xs font-medium text-studio-warn">Used but never defined</p>
                <p className="mt-1 text-xs text-studio-ink-soft">
            ...
            {!student && knowledge.disagreements.length > 0 && (
              <div className="mt-3 rounded border border-red-200 bg-red-50 px-3 py-2">
                <p className="text-xs font-medium text-studio-bad">Lectures that disagree</p>
                {knowledge.disagreements.map((d) => (
                  <p key={d.nodeId} className="mt-1 text-xs text-studio-ink-soft">
          ...
          <Card className="px-5 py-4">
            ...
            <ul className="mt-2 space-y-1 text-sm text-studio-ink-soft">
              {lecturers.map((l) => <li key={l.id}>{l.name}</li>)}
            </ul>
            {!student && (
              <p className="mt-3 text-xs text-studio-ink-faint">
