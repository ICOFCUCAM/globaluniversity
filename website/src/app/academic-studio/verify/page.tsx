...
import { verifyCertificate } from '@/academic/lib/service';
    ...
    <div className="mx-auto max-w-2xl px-6 py-12">
      ...
      <form className="mt-6 flex flex-wrap gap-2" method="get">
        <input
          ...
          className="min-w-[14rem] flex-1 rounded-md border border-studio-page-line px-3.5 py-2.5 font-mono text-sm uppercase"
        />
        <button type="submit" className="rounded-md bg-studio-brand px-4 py-2.5 text-sm font-medium text-white">
          Check
        </button>
      </form>

      {code && !found && (
        <p className="mt-6 rounded-md border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-studio-ink-soft">
      ...
      {found && (
        <article className={`mt-6 rounded-lg border px-6 py-5 ${
          found.revoked ? 'border-red-200 bg-red-50' : 'border-studio-page-line bg-studio-page-card'
        }`}>
          {found.revoked && (
            <p className="mb-3 font-medium text-studio-bad">
          ...
          <p className="text-sm text-studio-ink-soft">{found.courseTitle}</p>

          <ul className="mt-4 space-y-1.5 text-sm">
            {found.attests.map((line, i) => <li key={i}>— {line}</li>)}
          </ul>

          <p className="mt-4 text-xs text-studio-ink-faint">
