...
import { catalogue } from '@/academic/lib/service';
          ...
          <div className="grid gap-3 lg:grid-cols-2">
            {courses.map((course) => (
              <Card key={course.id} className="px-5 py-4" lang={course.originalLanguage}>
                {course.where && (
                  <p className="text-[11px] uppercase tracking-wide text-studio-ink-faint">{course.where}</p>
                )}
                ...
                <div className="mt-3 flex flex-wrap items-center gap-3 text-xs text-studio-ink-soft">
                  ...
                  {course.certifies && (
                    <span className="inline-flex items-center gap-1.5 text-studio-ok">
                ...
                <div className="mt-4 flex flex-wrap items-center gap-2">
                  {course.access === 'open' ? (
                    <Link
                      href={`/academic-studio/courses/${course.id}`}
                      className="rounded-md bg-studio-brand px-3.5 py-2 text-sm font-medium text-white hover:bg-studio-brand-dark"
                    >
                      {translate(reader, 'catalogue.open')}
                    </Link>
                  ) : (
                    <>
                      <span className="rounded-md border border-studio-page-line px-3.5 py-2 text-sm text-studio-ink-soft">
                        {course.price
