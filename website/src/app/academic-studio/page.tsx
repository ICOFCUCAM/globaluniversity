...
import { STAGES, STAGE_BY_KIND } from '@/academic/lib/pipeline/stages';
import { Card, Empty, PageHeader } from '@/academic/components/ui';
import { RegistryOverview } from '@/academic/components/RegistryOverview';
import type { Artefact, Lecture } from '@/academic/lib/domain/types';
...
export default async function Dashboard() {
  ...
  return (
    <div>
      {/*
        THE GATEWAY SAYS WHAT THE PLACE IS, once, above the personal greeting.
        The University asked for this line to be what a person sees on
        arriving, and it is the one sentence that explains the whole pipeline:
        a lecturer teaches, the model transforms, a student learns from what
        the lecturer approved — in that order, with the approval in the middle
        and not optional.
      */}
      <PageHeader
        eyebrow="Academic Studio · Lecturers teach. AI transforms. Students learn."
        ...
        actions={!student && courses.length > 0 && (
          <Link
            href={`/academic-studio/courses/${courses[0].id}/new-lecture`}
      ...
      <div className="px-6 py-6 md:px-8 space-y-6">
        {!student && awaitingReview > 0 && (
          <Card className="border-amber-200 bg-amber-50 px-5 py-4">
            <p className="text-sm font-medium text-studio-warn">
        ...
        <section>
          <h2 className="mb-3 text-sm font-semibold uppercase tracking-wide text-studio-ink-faint">
            ...
            <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
              {courses.map((course) => (
                <Link key={course.id} href={`/academic-studio/courses/${course.id}`}>
                  <Card className="h-full px-5 py-4 transition hover:border-studio-brand/40 hover:shadow-sm">
                    <p className="text-[11px] font-medium uppercase tracking-wide text-studio-brand">{course.code}</p>
          ...
          {rows.length === 0 ? (
            ...
            <div className="space-y-2">
              {rows.slice(0, 8).map(({ lecture, artefacts, courseCode }) => {
                ...
                return (
                  <Link key={lecture.id} href={`/academic-studio/lectures/${lecture.id}`}>
                    <Card className="flex flex-wrap items-center justify-between gap-3 px-5 py-4 transition hover:border-studio-brand/40">
                          ...
                          <span
                            ...
                            className={`rounded px-2 py-0.5 text-[11px] ${
                              published.has(stage.kind) ? 'bg-emerald-600 text-white'
                                : made.has(stage.kind) ? 'bg-studio-brand-tint text-studio-brand-dark'
                                  : 'bg-studio-page text-studio-ink-faint'
                            }`}
                          >
                            {stage.label.split(' ')[0]}
                          </span>
        ...
        {!student && (
          <Card className="px-5 py-4">
            <div className="flex items-start gap-3">
              <Mic size={18} className="mt-0.5 text-studio-ink-faint" />
              <div>
