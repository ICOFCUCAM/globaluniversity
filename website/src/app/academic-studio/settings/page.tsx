...
import { PLANS } from '@/academic/lib/billing/plans';
import { STAGES } from '@/academic/lib/pipeline/stages';
import { capabilitiesOf, ROLE_LABEL } from '@/academic/lib/capabilities';
import { can } from '@/academic/lib/capabilities';
import { InstitutionVoice } from '@/academic/components/InstitutionVoice';
import { Card, PageHeader } from '@/academic/components/ui';
    ...
    <div>
      ...
      <div className="grid gap-4 px-6 py-6 md:px-8 lg:grid-cols-2">
        <Card className="px-5 py-4">
          ...
          <dl className="mt-3 space-y-2 text-sm">
            <div className="flex justify-between gap-4">
              <dt className="text-studio-ink-soft">Language model</dt>
              <dd className="text-end font-medium">{wired.model}</dd>
            </div>
            <div className="flex justify-between gap-4">
              <dt className="text-studio-ink-soft">Transcription</dt>
              <dd className="text-end font-medium">{wired.transcription}</dd>
            </div>
            <div className="flex justify-between gap-4">
              <dt className="text-studio-ink-soft">Speech</dt>
              <dd className="text-end font-medium">{wired.speech}</dd>
            </div>
          </dl>
          {!wired.live && (
            <p className="mt-3 rounded border border-amber-200 bg-amber-50 px-3 py-2 text-xs text-studio-ink-soft">
        ...
        <Card className="px-5 py-4">
          ...
          <dl className="mt-3 space-y-2 text-sm">
            <div className="flex justify-between gap-4">
              <dt className="text-studio-ink-soft">Store</dt>
              <dd className="font-medium">{store.id === 'memory' ? 'In process (demonstration)' : store.id}</dd>
            </div>
            <div className="flex justify-between gap-4">
              <dt className="text-studio-ink-soft">Jobs pending</dt>
              <dd className="font-medium">{pending}</dd>
            </div>
            <div className="flex justify-between gap-4">
              <dt className="text-studio-ink-soft">Signed in as</dt>
          ...
          <ol className="mt-3 space-y-2 text-sm">
            {STAGES.map((stage, i) => (
              <li key={stage.kind} className="flex gap-3">
                <span className="w-5 shrink-0 text-studio-ink-faint">{i + 1}</span>
                <span>
                  <strong className="font-medium">{stage.label}</strong>
                  <span className="text-studio-ink-soft"> — {stage.purpose}</span>
                  {stage.requiresApprovedSource && (
                    <span className="ms-1 text-[11px] text-studio-warn">· waits for the lecturer’s approval</span>
                  )}
                </span>
              </li>
            ))}
        ...
        <Card className="px-5 py-4 lg:col-span-2">
          ...
          <div className="mt-3 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
            {PLANS.map((plan) => (
              <div key={plan.id} className="rounded border border-studio-page-line px-3 py-3">
                ...
                <p className="mt-2 text-xs text-studio-ink-soft">{plan.blurb}</p>
              </div>
            ))}
          </div>
        </Card>
