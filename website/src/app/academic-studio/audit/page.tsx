...
import { auditLog, Refused } from '@/academic/lib/service';
import { AUDITED_ACTS, describe } from '@/academic/lib/audit/audit';
      ...
      <div className="px-6 py-6 md:px-8">
        <Card className="mb-4 px-5 py-4">
          <h2 className="flex items-center gap-2 text-sm font-semibold">
            <ShieldCheck size={16} className="text-studio-brand" /> What is kept
          </h2>
        ...
        {searchParams.course && (
          <p className="mb-3 text-xs text-studio-ink-soft">
            Showing {named(searchParams.course) ?? 'one course'} only ·{' '}
            <Link href="/academic-studio/audit" className="text-studio-brand hover:underline">everything you may read</Link>
          </p>
        )}
              ...
              <li key={entry.id}>
                <Card className="px-5 py-3">
                  <div className="flex flex-wrap items-baseline justify-between gap-2">
                    ...
                    <p className="text-[11px] text-studio-ink-faint">
                      {new Date(entry.at).toLocaleString()}
                      {entry.courseId && ` · ${named(entry.courseId) ?? entry.courseId}`}
                    </p>
                  </div>
                  {entry.detail && (
                    <p className="mt-1 text-xs text-studio-ink-soft">“{entry.detail}”</p>
                  )}
                </Card>
              </li>
            ))}
