...
import { Card, Empty, PageHeader, StateBadge } from '@/academic/components/ui';
        ...
        {rows.length === 0
          ...
          : rows.map(({ course, lecture, artefacts }) => (
            <Link key={lecture.id} href={`/academic-studio/lectures/${lecture.id}`}>
              <Card className="flex flex-wrap items-center justify-between gap-3 px-5 py-4 hover:border-studio-brand/40">
                <div>
                ...
                <div className="flex flex-wrap items-center gap-1.5">
                  {artefacts.map((a) => (
                    <span key={a.id} className="flex items-center gap-1 text-[11px] text-studio-ink-faint">
