            ...
            <ul className="mt-10 space-y-9">
              {PROMISES.map((p, i) => (
                <Reveal key={p.title} delay={120 + i * 100}>
                  <li className="relative border-l-2 border-brand-sand pl-7 transition-colors duration-500 hover:border-brand-gold">
                    <h3 className="font-heading text-xl font-bold text-brand-purple">{p.title}</h3>
                    <p className="mt-2.5 leading-relaxed text-brand-muted">{p.body}</p>
                  </li>
                </Reveal>
              ))}
            </ul>
