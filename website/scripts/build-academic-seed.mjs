...
w();
w('  -- ---------------------------------------------------------------------');
w('  -- NO ASSERTION ABOUT WHAT DOES NOT EXIST YET.');
w('  --');
w('  -- This checked twice, and was wrong twice. First it asserted that no');
w('  -- programme VERSION existed — true until 061 seeded one for every');
w('  -- programme. Rescoped to curriculum ENTRIES, it was true until 062 moved');
w('  -- three curricula in. Each time the bundle was clean on its first pass');
w('  -- from empty and red on its second.');
w('  --');
w('  -- The mistake is the shape, not the table. A migration cannot prove');
w('  -- anything by counting rows it did not create: every later migration is');
w('  -- free to create them, and a proof that depends on the future being empty');
w('  -- is a proof with an expiry date. 045 collided with 055 the same way.');
w('  --');
w('  -- What 060 is answerable for is above: the schools and programmes IT');
w('  -- seeded, and that not one of them is open. That is the whole of its job.');
w('  -- ---------------------------------------------------------------------');
w();
